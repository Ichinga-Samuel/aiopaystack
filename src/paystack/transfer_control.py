...
class TransferControl(Base):
    ...
    async def resend_transfer_otp(self, *, transfer_code: str, reason: Literal['resend_otp', 'transfer']):
        ...
        :return:
        """
        return await self.post(url=self.url(""), json={'transfer_code': transfer_code, 'reason': reason})
    ...
    async def finalize_disable_otp(self, *, otp: str):
        ...
        :return:
        """
        return await self.post(url=self.url("transfer/disable_otp_finalize"), json={'otp': otp})
